...
url = 'https://www.gamberorosso.it/ristoranti/#1602496015233-6fac7d3f-ee84'
response = requests.get(url, headers=headers)
response.raise_for_status()

soup = BeautifulSoup(response.text, 'html.parser')
main_div = soup.find('div', id='1602496015233-6fac7d3f-ee84')

references = [a.get('href') for a in main_div.find_all('a', class_='_self cvplbd')]

json_output = { 'Tre Gamberi Gambero Rosso 2024': [] }
...
for ref in references:
    ...
    json_output['Tre Gamberi Gambero Rosso 2024'].append({
        'category': category,
        'position': None,
        'name': name.text,
        'ref': ref,
...
with open('treGamberiGamberoRosso.json', 'w', encoding='utf-8') as file:
